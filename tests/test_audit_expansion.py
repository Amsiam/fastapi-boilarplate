...
async def test_role_audit(role_service, actor_id):
    ...
    # Verify Create Log
    db = mongodb.get_db()
    logs = await db["audit_logs"].find({"target_id": role["id"], "action": "create_role"}).to_list(length=1)
    assert len(logs) == 1
    ...
    # Verify Update Log
    logs = await db["audit_logs"].find({"target_id": role["id"], "action": "update_role"}).to_list(length=1)
    assert len(logs) == 1
    ...
    # Verify Delete Log
    logs = await db["audit_logs"].find({"target_id": role["id"], "action": "delete_role"}).to_list(length=1)
    assert len(logs) == 1
...
async def test_permission_audit(perm_service, actor_id):
    ...
    # Verify Create Log
    db = mongodb.get_db()
    logs = await db["audit_logs"].find({"target_id": perm["id"], "action": "create_permission"}).to_list(length=1)
    assert len(logs) == 1
    ...
    # Verify Delete Log
    logs = await db["audit_logs"].find({"target_id": perm["id"], "action": "delete_permission"}).to_list(length=1)
    assert len(logs) == 1
