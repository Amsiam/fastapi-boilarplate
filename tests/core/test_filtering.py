...
async def test_apply_filters_eq(session):
    ...
    u1 = await repo.create(User(email="eq_test_a@example.com", hashed_password="pw", role=UserRole.ADMIN))
    u2 = await repo.create(User(email="eq_test_b@example.com", hashed_password="pw", role=UserRole.CUSTOMER))
    u3 = await repo.create(User(email="eq_test_c@example.com", hashed_password="pw", role=UserRole.CUSTOMER))
    
    # Test strict equality
    # We filter by both role AND our unique test marker to avoid pollution
    items, total = await repo.get_list(filters={"role": UserRole.CUSTOMER, "email__like": "eq_test_"})
    
    assert total == 2
    assert len(items) == 2
    assert all(u.role == UserRole.CUSTOMER for u in items)
...
async def test_apply_filters_search(session):
    ...
    await repo.create(User(email="unique1@test.com", hashed_password="pw", role=UserRole.CUSTOMER, first_name="Zorro", last_name="X"))
    await repo.create(User(email="other2@test.com", hashed_password="pw", role=UserRole.CUSTOMER, first_name="Arthur", last_name="Y"))
...
async def test_apply_sorting(session):
    ...
    await repo.create(User(email="sort_a@test.com", hashed_password="pw", role=UserRole.CUSTOMER, first_name="A", last_name="SortTest"))
    await repo.create(User(email="sort_b@test.com", hashed_password="pw", role=UserRole.CUSTOMER, first_name="B", last_name="SortTest"))
